...
#if UNITY_2019_3_OR_NEWER
        private const int PackedAssetsPerPage = 1; // Number of packed assets to display per page
        private const int ItemsPerPage = 20; // Number of items to display per page
        private int currentPackedAssetsPage = 0; // Current page index for packed assets
        private Dictionary<string, int> currentItemsPage = new Dictionary<string, int>(); // Current page index for items within each packed asset

        private void OnAssetsGUI()
        {
            var vPos = -scrollPosition.y;

            // Pagination controls for packed assets
            int totalPackedAssetsPages = Mathf.CeilToInt((float)report.packedAssets.Length / PackedAssetsPerPage);
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Previous Packed Assets", GUILayout.Width(150)) && currentPackedAssetsPage > 0)
            {
                currentPackedAssetsPage--;
            }
            GUILayout.FlexibleSpace();
            GUILayout.Label($"Packed Assets Page {currentPackedAssetsPage + 1} of {totalPackedAssetsPages}");
            GUILayout.FlexibleSpace();
            if (GUILayout.Button("Next Packed Assets", GUILayout.Width(150)) && currentPackedAssetsPage < totalPackedAssetsPages - 1)
            {
                currentPackedAssetsPage++;
            }
            GUILayout.EndHorizontal();

            // Display packed assets for the current page
            int packedAssetsStartIndex = currentPackedAssetsPage * PackedAssetsPerPage;
            int packedAssetsEndIndex = Mathf.Min(packedAssetsStartIndex + PackedAssetsPerPage, report.packedAssets.Length);
            var paginatedPackedAssets = report.packedAssets.Skip(packedAssetsStartIndex).Take(packedAssetsEndIndex - packedAssetsStartIndex);

            foreach (var packedAsset in paginatedPackedAssets)
            {
                ulong totalPackedAssetSize = (ulong)packedAsset.contents.Sum(entry => (float)entry.packedSize);

                GUILayout.Label($"Packed Asset: {packedAsset.shortPath} (Total Size: {FormatSize(totalPackedAssetSize)})");

                // Initialize current page index for items within this packed asset if not already initialized
                if (!currentItemsPage.ContainsKey(packedAsset.shortPath))
                {
                    currentItemsPage[packedAsset.shortPath] = 0;
                }

                // Pagination controls for items within the packed asset
                int totalItemsPages = Mathf.CeilToInt((float)packedAsset.contents.Length / ItemsPerPage);
                GUILayout.BeginHorizontal();
                if (GUILayout.Button("Previous Items", GUILayout.Width(100)) && currentItemsPage[packedAsset.shortPath] > 0)
                {
                    currentItemsPage[packedAsset.shortPath]--;
                }
                GUILayout.FlexibleSpace();
                GUILayout.Label($"Items Page {currentItemsPage[packedAsset.shortPath] + 1} of {totalItemsPages}");
                GUILayout.FlexibleSpace();
                if (GUILayout.Button("Next Items", GUILayout.Width(100)) && currentItemsPage[packedAsset.shortPath] < totalItemsPages - 1)
                {
                    currentItemsPage[packedAsset.shortPath]++;
                }
                GUILayout.EndHorizontal();

                // Display items for the current page within the packed asset
                int itemsStartIndex = currentItemsPage[packedAsset.shortPath] * ItemsPerPage;
                int itemsEndIndex = Mathf.Min(itemsStartIndex + ItemsPerPage, packedAsset.contents.Length);
                var paginatedItems = packedAsset.contents.Skip(itemsStartIndex).Take(itemsEndIndex - itemsStartIndex);

                ShowAssets(paginatedItems.Select(entry => new AssetEntry
                {
                    size = (int)entry.packedSize,
                    icon = AssetDatabase.GetCachedIcon(entry.sourceAssetPath),
                    outputFile = packedAsset.shortPath,
                    type = AssetImporter.GetAtPath(entry.sourceAssetPath)?.GetType().Name ?? "Unknown",
